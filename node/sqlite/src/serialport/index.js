...
async function main() {
    ...
    try {
        ...
        // 读取米数
        const result = await meterReader.readMeters();
        console.log("=== 米数表读数 ===");
        console.log("原始寄存器值:", result.rawValue);
        console.log("计算米数:", result.meters.toFixed(3) + "米");
        console.log("显示值:", result.displayValue.toFixed(1) + "米");
        console.log("当前的单位：", result.unit);
        // 持续读取（可选）
        console.log("\n开始持续读取（按Ctrl+C停止）...");
        setInterval(async () => {
            try {
                const reading = await meterReader.readMeters();
                console.log(`当前米数: ${reading.displayValue.toFixed(2)}米----原始米数: ${reading.rawValue}`);
            }
            catch (error) {
